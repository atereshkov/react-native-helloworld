...
const Overlay = ({children, visible}) => (
  (visible) ? (
    <View style={{position: 'absolute', marginTop: (Platform.OS === 'ios') ? 48 : 28, zIndex: 1}}>
      ...
      <View style={{padding: 10, marginTop: -17, width: Dimensions.get('window').width, height: 300, backgroundColor: 'rgba(255,255,255,80)'}}>
      {children}
      </View>
    </View>
    ) : <View />
        ...
        <View style={styles.body}>
          ...
          <Overlay visible={overlayVisible}>
            <Text style={{fontSize: 18}}>
              <Text style={{fontWeight: 'bold'}}>NewsReader</Text> is a simple news reader for the Web and a React Native app (Android / iOS).
            </Text>
            <Text style={{fontSize: 18, marginTop: 20}}>
              Made with ❤
              <Text
                onPress={() => { onOpenUrl('http://google.com/') }}>
                <Text style={[styles.aboutLink, cursorStyle]}>{" "}todo</Text>
              </Text>
              , source code:
              <Text
                onPress={() => { onOpenUrl('http://google.com/') }}>
                <Text style={[styles.aboutLink, cursorStyle]}>{" "}(todo url)</Text>
              </Text>
            </Text>
          </Overlay>
